...
app.post('/addPreset', (req, res) => {
    ...
    if (name && name.trim() !== '') {
        const preset = { name, colors: Array.from(colorArray) };
        presets.push(preset);
        savePresetsToFile();
...
app.post('/applyPreset', async (req, res) => {
    ...
    if (selectedPreset) {
        ...
        res.status(400).json({ message: 'Preset not found' });
    }
});
...
app.post('/toggleLights', (req, res) => {
    ...
    if (body.token == "48FJSNnc893SLFJ@41%s5#") {
        if (body.lightsOn) {
            ...
            var chosenPreset = activePreset == 'White' ? 'Red' : 'White';
